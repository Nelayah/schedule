    ...
    render() {
        ...
        return (React.createElement("div", { className: classnames(cellColumnsCls, { [`${cellColumnsCls}-active`]: isActive }), onMouseDown: this.handleCellBlocksActive, onMouseUp: this.handleCellBlocksInActive, onMouseLeave: this.handleCellBlocksInActive, style: { width: cellWidth } },
            ...
            this.selectedAreas.map(v => {
                return (React.createElement(Blocks, { key: `${v.rowStartKey}_${v.rowEndKey}`, data: v, top: TimeProcessor.calculateTimeLen(v.rowStartKey, rows[0].key, timelineInterval, cellHeight) + cellHeight + 1, height: TimeProcessor.calculateTimeLen(v.rowEndKey, v.rowStartKey, timelineInterval, cellHeight) - 2, unit: unit, forbidden: readonly }));
            })));
    }
};
