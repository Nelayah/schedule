...
export const defaultArray = R.defaultTo([]);
// 列项极限参考值值
...
export const countOverlaps = times => {
    ...
    for (let i = 0; i < len; i++)
        times[i]._overlay = times[i]._overlayIndex.map(k => ({ _track: times[k]._track }));
    return ({
        times,
        tracks
    });
...
export const preProcessing = R.compose(countOverlaps, R.sort((x, y) => x._startTime - y._startTime), R.map((v) => {
    return R.compose(R.mergeDeepLeft({
        _track: 0,
        _disabledTrack: [],
        _overlay: [],
        _overlayIndex: [],
...
// 找出重叠区域最近右边区域
export const findTheMinimumGreater = v => R.compose(R.reduce(R.min, MAX_REF_NUMBER), 
// @ts-ignore
R.filter(R.gt(R.__, v._track)), R.pluck('_track'), R.prop('_overlay'))(v);
